...
?>
    ...
    {{ csrf_field() }}
    	...
    	<input type="submit" name="submission" value="Save Changes" onclick="return check('edit', 'job history')"></input>
    </form>
    <form action="deleteJobHistory" method="post">
		{{ csrf_field() }}
		<input type="hidden" name="userInfoID" value="<?php echo $profileID?>"></input>
    				<input type="hidden" name="id" value="<?php echo $id;?>"></input>
    				<input type="submit" name="submission" style="background-color: red; color: white;" value="Delete Job" onclick="return check('delete', 'job history')"></input>
